...
class SellSimulator:
    def __init__(
        ...
        self.stoploss_params = (
            stoploss_params
            if stoploss_params is not None
            else {
...
def save_results(trades, metrics, timestamp, params=None, is_grid_test=False):
    ...
    if params and "momentum_params" in params and params["momentum_params"]:
        ...
        filename = (
            f"momentum_tp{params['base_take_profit']}_sl{params['stop_loss']}_ts{params['trailing_stop']}"
        ...
        # Perusparametrit
        filename = (
            f"tp{params['base_take_profit']}_sl{params['stop_loss']}_ts{params['trailing_stop']}_{timestamp}"
            if params
            else f"results_{timestamp}"
        )
    ...
    if not is_grid_test:
        ...
        if params:
            print(
                f"Parametrit: TP={params['base_take_profit']}, SL={params['stop_loss']}, TS={params['trailing_stop']}"
            )
        ...
        print("\n=== MYYNTITYYPIT ===")
        print(
            f"Stop Loss -kauppoja: {metrics['stoploss_trades']} ({metrics['stoploss_trades']/metrics['total_trades']*100:.1f}%)"
        )
        print(
            f"Take Profit -kauppoja: {metrics['tp_trades']} ({metrics['tp_trades']/metrics['total_trades']*100:.1f}%)"
        )
        print(
            f"Low Performance -kauppoja: {metrics['lp_trades']} ({metrics['lp_trades']/metrics['total_trades']*100:.1f}%)"
        )
        print(
            f"Force Sell -kauppoja: {metrics['force_trades']} ({metrics['force_trades']/metrics['total_trades']*100:.1f}%)"
        )
        ...
        [
            {
                ...
                "mc_change_threshold": (
                    r["parameters"]["momentum_params"]["mc_change_threshold"]
                    if "momentum_params" in r["parameters"]
                    else 0
                ),
                "holder_change_threshold": (
                    r["parameters"]["momentum_params"]["holder_change_threshold"]
                    if "momentum_params" in r["parameters"]
                    else 0
                ),
                "buy_volume_threshold": (
                    r["parameters"]["momentum_params"]["buy_volume_threshold"]
                    if "momentum_params" in r["parameters"]
                    else 0
                ),
                "net_volume_threshold": (
                    r["parameters"]["momentum_params"]["net_volume_threshold"]
                    if "momentum_params" in r["parameters"]
                    else 0
                ),
                "required_strong": (
                    r["parameters"]["momentum_params"]["required_strong"] if "momentum_params" in r["parameters"] else 0
                ),
                "lp_holder_growth_threshold": (
                    r["parameters"]["momentum_params"].get("lp_holder_growth_threshold", 0)
                    if "momentum_params" in r["parameters"]
                    else 0
                ),
...
def run_sell_simulation(summary_file=None, grid_test=False, param_combinations=None, timestamp=None):
    ...
    if summary_file:
        ...
        opportunity_files = [
            os.path.join(buy_opportunities_dir, f) for f in os.listdir(buy_opportunities_dir) if f.endswith(".pkl")
        ]
    ...
    if grid_test and param_combinations:
        ...
        for i, params in enumerate(param_combinations):
            ...
            simulator_params = {"initial_investment": 1.0}
...
def run_simulation_with_params(
    ...
    print(
        f"Jaettu {total_opportunities} ostomahdollisuutta {len(opportunity_batches)} erään, noin {batch_size} per erä"
    )
    ...
    for i, batch in enumerate(opportunity_batches):
        p = Process(
            target=process_opportunity_batch,
        ...
        while any(p.is_alive() for p in processes):
            try:
                ...
                if opportunities_processed > 0:
                    ...
                    print(
                        f"\rEdistyminen: {opportunities_processed}/{total_opportunities} ostomahdollisuutta käsitelty "
                        ...
                        + f"Arvioitu jäljellä: {estimated_time_left/60:.1f} min",
                        end="",
                    )
