...
def get_parameters():
    ...
    return {
        ...
        'mc_growth_from_start': [50, 100, 150],  # Markkinakapitalisaation kasvu alusta (%)
...
class BuySimulator:
    ...
    def check_buy_conditions(self, metrics: Dict[str, float], initial_metrics: Dict[str, float], pool_data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if conditions_met >= required_conditions:
                ...
                print(f"Ostoehdot eivät täyty: vain {conditions_met}/{len(self.buy_params)} parametria ylittää raja-arvot")
                return False
        ...
        try:
            ...
            try:
                ...
                required_columns = ['marketCap', 'timestamp', 'poolAddress', 
                                  ...
                                  'holderDelta5s', 'holderDelta10s', 'holderDelta30s', 'holderDelta60s',
                                  'holdersCount', 'buyVolume5s', 'buyVolume10s',
                                  'netVolume5s', 'netVolume10s', 'priceChangePercent',
                                  'trade_last10Seconds.volume.sell',
                                  'currentPrice', 'minMarketCap', 'athMarketCap',
                                  'maMarketCap30s', 'maMarketCap60s']
                ...
                for delay in range(self.min_delay, self.max_delay + 1):
                    ...
                    try:
                        ...
                        current_metrics = {
                            ...
                            'holder_growth_from_start': pool_data.iloc[delay]['holdersCount'] - initial_metrics['holdersCount'],
                            'mc_growth_from_start': ((pool_data.iloc[delay]['marketCap'] / initial_metrics['marketCap']) - 1) * 100,
...
def preprocess_pool_data(df):
    ...
    required_columns = ['marketCap', 'holdersCount', 'buyVolume5s', 'buyVolume10s', 
                        'trade_last5Seconds.volume.sell', 'trade_last10Seconds.volume.sell']
...
def calculate_metrics(buy_opportunities):
    ...
    weighted_return = (0.7 * avg_realistic_sol_return) + (0.3 * opportunity_weight * 10)  # Kerrotaan 10:llä skaalauksen vuoksi
...
def create_return_report(buy_opportunities, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"alle 1x: {metrics['return_distribution']['under_1x']} kpl ({metrics['return_distribution_percentage']['under_1x']:.1f}%)\n")
        f.write(f"1-1.8x: {metrics['return_distribution']['1x_to_1_8x']} kpl ({metrics['return_distribution_percentage']['1x_to_1_8x']:.1f}%)\n")
        f.write(f"1.8-3x: {metrics['return_distribution']['1_8x_to_3x']} kpl ({metrics['return_distribution_percentage']['1_8x_to_3x']:.1f}%)\n")
        f.write(f"3-4x: {metrics['return_distribution']['3x_to_4x']} kpl ({metrics['return_distribution_percentage']['3x_to_4x']:.1f}%)\n")
        f.write(f"4-5x: {metrics['return_distribution']['4x_to_5x']} kpl ({metrics['return_distribution_percentage']['4x_to_5x']:.1f}%)\n")
        f.write(f"5-6x: {metrics['return_distribution']['5x_to_6x']} kpl ({metrics['return_distribution_percentage']['5x_to_6x']:.1f}%)\n")
        f.write(f"6-7x: {metrics['return_distribution']['6x_to_7x']} kpl ({metrics['return_distribution_percentage']['6x_to_7x']:.1f}%)\n")
        f.write(f"7-8x: {metrics['return_distribution']['7x_to_8x']} kpl ({metrics['return_distribution_percentage']['7x_to_8x']:.1f}%)\n")
        f.write(f"8-9x: {metrics['return_distribution']['8x_to_9x']} kpl ({metrics['return_distribution_percentage']['8x_to_9x']:.1f}%)\n")
        f.write(f"9-10x: {metrics['return_distribution']['9x_to_10x']} kpl ({metrics['return_distribution_percentage']['9x_to_10x']:.1f}%)\n")
        f.write(f"10x+: {metrics['return_distribution']['over_10x']} kpl ({metrics['return_distribution_percentage']['over_10x']:.1f}%)\n\n")
        ...
        for i, opp in enumerate(sorted_opportunities):
            ...
            f.write(f"  Realistinen tuotto: {opp.get('realistic_return_percentage', 0):.2f}% ({opp.get('realistic_return', 0):.2f}x)\n")
...
def run_buy_simulation(df, buy_params=None, run_grid_test=False, continue_from=None):
    ...
    # Luo aikaleima
        ...
        grid_output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "grid_test_results")
        ...
        try:
            ...
            for i, combination in enumerate(param_combinations[start_index:], start=start_index):
                ...
                if buy_opportunities:
                    ...
                    print("\n=== TUOTTOJAKAUMA ===")
                    print(f"alle 1x: {metrics['return_distribution']['under_1x']} kpl ({metrics['return_distribution_percentage']['under_1x']:.1f}%)")
                    print(f"1-1.8x: {metrics['return_distribution']['1x_to_1_8x']} kpl ({metrics['return_distribution_percentage']['1x_to_1_8x']:.1f}%)")
                    print(f"1.8-3x: {metrics['return_distribution']['1_8x_to_3x']} kpl ({metrics['return_distribution_percentage']['1_8x_to_3x']:.1f}%)")
                    print(f"3-4x: {metrics['return_distribution']['3x_to_4x']} kpl ({metrics['return_distribution_percentage']['3x_to_4x']:.1f}%)")
                    print(f"4-5x: {metrics['return_distribution']['4x_to_5x']} kpl ({metrics['return_distribution_percentage']['4x_to_5x']:.1f}%)")
                    print(f"5-6x: {metrics['return_distribution']['5x_to_6x']} kpl ({metrics['return_distribution_percentage']['5x_to_6x']:.1f}%)")
                    print(f"6-7x: {metrics['return_distribution']['6x_to_7x']} kpl ({metrics['return_distribution_percentage']['6x_to_7x']:.1f}%)")
                    print(f"7-8x: {metrics['return_distribution']['7x_to_8x']} kpl ({metrics['return_distribution_percentage']['7x_to_8x']:.1f}%)")
                    print(f"8-9x: {metrics['return_distribution']['8x_to_9x']} kpl ({metrics['return_distribution_percentage']['8x_to_9x']:.1f}%)")
                    print(f"9-10x: {metrics['return_distribution']['9x_to_10x']} kpl ({metrics['return_distribution_percentage']['9x_to_10x']:.1f}%)")
                    print(f"10x+: {metrics['return_distribution']['over_10x']} kpl ({metrics['return_distribution_percentage']['over_10x']:.1f}%)")
                ...
                print(f"Parametriyhdistelmä {i+1}: Keskimääräinen tuotto {avg_return:.2f}%, "
            ...
            if results:
                ...
                summary_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "summary_reports")
            ...
            print(f"Edistyminen tallennettu. Voit jatkaa myöhemmin käynnistämällä ohjelman uudelleen tai käyttämällä --continue parametria.")
...
def run_single_simulation(df, buy_params, timestamp_suffix="", continue_from=None):
    ...
    output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "buy_opportunities1")
    ...
    for i in range(0, len(pools_to_process), batch_size):
        ...
        p = mp.Process(target=process_batch, args=(df, batch_pools, buy_params, results_queue, progress_queue))
        processes.append(p)
        p.start()
        ...
        while active_processes > 0:
            ...
            try:
                progress_msg = progress_queue.get(timeout=0.1)
                    ...
                    pool_id = progress_msg.split(":", 1)[1]
                    processed_pools.append(pool_id)
                    elif progress_msg.startswith("SKIP:"):
                    skipped_pools += 1
                    processed_count += 1
                    pool_id = progress_msg.split(":", 1)[1]
                    processed_pools.append(pool_id)
                    elif progress_msg.startswith("BATCH_ERROR:"):
                    error_msg = progress_msg.split(":", 1)[1]
                    print(f"Erän käsittelyssä virhe: {error_msg}")
            except Empty:
                pass
            ...
            if total_processed > 0 and total_processed % 1 == 0:
                ...
                print(f"\rEdistyminen: {processed_count}/{total_to_process} poolia käsitelty "
                      ...
                      f"CPU: {cpu_percent:.1f}%, RAM: {ram_percent:.1f}%", end="")
    ...
    progress_files = glob.glob(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                                           "progress", f"*{'grid' if timestamp_suffix else 'normal'}*.json"))
...
def get_latest_progress_file(grid_test=False):
    ...
    progress_files = [f for f in os.listdir(progress_dir) if f.startswith('progress_') and f.endswith(f'{grid_suffix}.json')]
...
def check_previous_run(grid_test=False, force_new=False):
    ...
    print(f"Edistyminen: {progress_data.get('pools_processed', 0)}/{progress_data.get('total_pools', 0)} poolia käsitelty.")
    
    choice = input("Haluatko jatkaa tästä ajosta vai aloittaa uuden?\n1: Jatka keskeneräistä ajoa\n2: Aloita uusi ajo\nValintasi: ")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--grid', action='store_true', help='Suorita grid-testi useilla parametriyhdistelmillä')
    ...
    parser.add_argument('--params', type=str, help='Käytettävät parametrit JSON-muodossa (esim. \'{"net_volume_5s": 0.2, "price_change": 0.03}\')')
