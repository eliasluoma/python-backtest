...
class FirebaseService:
    ...
    def get_available_pools(self, limit: Optional[int] = None, collection_name: str = "marketContext") -> List[str]:
        """
        ...
        try:
            return get_pool_ids(self.db, limit=limit, collection_name=collection_name)
        except Exception as e:
    ...
    def _get_single_pool_datapoints_count(self, pool_id: str, collection_name: str = "marketContext") -> int:
        """
        Internal method to get the number of datapoints for a single pool.
        Uses the specified collection_name to find the pool document and its marketContexts subcollection.

        Args:
            pool_id: Pool ID to check
            collection_name: Name of the Firestore collection where the pool document resides.

        Returns:
            Integer count of datapoints
        """
        try:
            logger.debug(f"Starting datapoint calculation for pool {pool_id} in collection '{collection_name}'")

            # Primary path: /<collection_name>/[pool-id]/marketContexts/
            logger.debug(f"Checking primary path: /{collection_name}/{pool_id}/marketContexts/")

            contexts_collection_ref = self.db.collection(collection_name).document(pool_id).collection("marketContexts")

            # Try to fetch one document to confirm existence and then count
            try:
                docs_snapshot = list(contexts_collection_ref.limit(1).stream())
                if not docs_snapshot:
                    logger.debug(f"No documents found at path /{collection_name}/{pool_id}/marketContexts/")
                    return 0

                logger.debug(
                    f"Document(s) found at path /{collection_name}/{pool_id}/marketContexts/. Proceeding to count."
                )

                # Try Firebase count() aggregate if available (more efficient)
                try:
                    if hasattr(contexts_collection_ref, "count") and callable(
                        getattr(contexts_collection_ref, "count")
                    ):
                        count_query = contexts_collection_ref.count()
                        ...
                        if (
                            ...
                            logger.debug(
                                f"Found {data_count} datapoints for pool {pool_id} in /{collection_name}/{pool_id}/marketContexts/ using count() API."
                            )
                            return data_count
                        else:
                            logger.debug("count() API did not return a valid result. Falling back to streaming docs.")
                except Exception as e_count_api:
                    logger.debug(
                        f"count() API failed for /{collection_name}/{pool_id}/marketContexts/: {e_count_api}. Falling back."
                    )

                # Fallback: Fetch all document IDs (up to a limit) and count them
                # This is less efficient for very large subcollections.
                all_docs_stream = contexts_collection_ref.limit(50000).stream()  # Increased limit for safety
                data_count = sum(1 for _ in all_docs_stream)  # Efficiently count items in a stream

                logger.debug(
                    f"Found {data_count} datapoints for pool {pool_id} in /{collection_name}/{pool_id}/marketContexts/ by streaming document IDs (limit 50000)."
                )
                if data_count > 0 and data_count < 50000:  # Log first doc only if not hitting limit
                    # Re-fetch one to log, as stream is consumed
                    first_doc_snapshot = list(contexts_collection_ref.limit(1).stream())
                    if first_doc_snapshot:
                        logger.debug(
                            f"First datapoint ID: {first_doc_snapshot[0].id}, Data: {first_doc_snapshot[0].to_dict()}"
                        )
                elif data_count == 50000:
                    logger.warning(
                        f"Reached document stream limit (50000) for pool {pool_id} in /{collection_name}/{pool_id}/marketContexts/. Count may be higher."
                    )
                return data_count

            except Exception as e_primary_path:
                logger.debug(
                    f"Error accessing or counting in primary path /{collection_name}/{pool_id}/marketContexts/: {e_primary_path}"
                )
                # If the primary path itself fails (e.g., pool document doesn't exist in collection_name), return 0
                return 0

        except Exception as e_outer:
            logger.error(
                f"Overall error calculating datapoints for pool {pool_id} in collection '{collection_name}': {e_outer}"
            )
            return 0

    def get_first_and_last_document_id(self, pool_id: str, collection_name: str = "marketContext") -> tuple:
        """
        Fetches the first and last market context document IDs for a pool from a specific collection.
        ...
        try:
            # Primary path to fetch from <collection_name>/<pool_id>/marketContexts subcollection
            contexts_ref = self.db.collection(collection_name).document(pool_id).collection("marketContexts")

            # Check if data exists at this path by trying to get one document
            # Order by timestamp to get the actual first document
            first_doc_query_check = contexts_ref.order_by("timestamp", direction="ASCENDING").limit(1)
            test_docs = list(first_doc_query_check.stream())

            if not test_docs:
                logger.debug(
                    f"No data found at path {collection_name}/{pool_id}/marketContexts. "
                    f"Checking for {pool_id}_* pattern directly in '{collection_name}' (if it's 'marketContext')."
                )
                # Fallback: Only if collection_name is the default 'marketContext',
                # try direct access to the collection assuming IDs might be {pool_id}_timestamp
                if collection_name == "marketContext":
                    direct_contexts_ref = self.db.collection(collection_name)
                    prefix = f"{pool_id}_"

                    # Query for first document by ID
                    first_doc_query_fallback = (
                        direct_contexts_ref.where("__name__", ">=", prefix)
                        .where("__name__", "<=", prefix + "\uf8ff")
                        .order_by("__name__", direction="ASCENDING")
                        .limit(1)
                    )
                    first_docs_fallback = list(first_doc_query_fallback.stream())

                    if not first_docs_fallback:
                        logger.debug(
                            f"No first document found for pool {pool_id} directly in '{collection_name}' collection with prefix '{prefix}'."
                        )
                        return None, None

                    first_doc_fb = first_docs_fallback[0]
                    first_id_fb = first_doc_fb.id

                    # Query for last document by ID
                    try:
                        from google.cloud.firestore_v1 import Query as FirestoreQuery  # Alias to avoid conflict

                        last_doc_query_fallback = (
                            direct_contexts_ref.where("__name__", ">=", prefix)
                            .where("__name__", "<=", prefix + "\uf8ff")
                        ...
                        # Fallback if specific Query.DESCENDING is an issue (shouldn't be with modern SDKs)
                        logger.warning(
                            "google.cloud.firestore_v1.Query.DESCENDING not available, using string 'DESCENDING'."
                        )
                        last_doc_query_fallback = (
                            direct_contexts_ref.where("__name__", ">=", prefix)
                            .where("__name__", "<=", prefix + "\uf8ff")
                    ...
                    last_docs_fallback = list(last_doc_query_fallback.stream())
                    if not last_docs_fallback:
                        logger.debug(
                            f"No last document found for pool {pool_id} directly in '{collection_name}' with prefix '{prefix}'."
                        )
                        return first_id_fb, None  # Still return first_id if found

                    last_doc_fb = last_docs_fallback[0]
                    last_id_fb = last_doc_fb.id
                    logger.debug(
                        f"Documents found for pool {pool_id} in '{collection_name}' (direct ID match) between {first_id_fb} - {last_id_fb}."
                    )
                    return first_id_fb, last_id_fb
                else:
                    logger.debug(
                        f"Skipping direct collection ID pattern access for non-default collection_name: '{collection_name}'. No data in primary path."
                    )
                    return None, None

            # If test_docs were found, it means the primary path is valid.
            # The first document from test_docs is the actual first chronological document.
            first_doc = test_docs[0]
            first_id = first_doc.id

            # Get the last document by timestamp (newest) from the primary path
            try:
                from google.cloud.firestore_v1 import Query as FirestoreQuery  # Alias

                last_doc_query = contexts_ref.order_by("timestamp", direction=FirestoreQuery.DESCENDING).limit(1)
            except (ImportError, AttributeError):
                logger.warning("google.cloud.firestore_v1.Query.DESCENDING not available, using string 'DESCENDING'.")
            ...
            if not last_docs:
                logger.debug(
                    f"No last document found for pool {pool_id} in {collection_name}/{pool_id}/marketContexts (should not happen if first was found)."
                )
                return first_id, None  # Return first_id if first was found but last wasn't (edge case)

            last_doc = last_docs[0]
            last_id = last_doc.id

            logger.debug(
                f"Documents found for pool {pool_id} in {collection_name}/{pool_id}/marketContexts between {first_id} - {last_id}."
            )
            return first_id, last_id

        except Exception as e:
            logger.error(f"Error retrieving document IDs for pool {pool_id} from collection '{collection_name}': {e}")
            return None, None
